...
class Scope {
	...
	parentOf(scope) {
		return this.length !== scope.length && this.shared(scope).length === this.length;
	}
	...
	in(scope) {
		return this.parentOf(scope) || this.isSame(scope);
	}

	childOf(scope) {
		return this.length !== scope.length && this.shared(scope).length === scope.length;
	}
