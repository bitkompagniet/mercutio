...
const roleStringEquality = require('./role-string-equality');
...
class Role {
	...
	hasSameRole(role) {
		return roleStringEquality(this._role, role._role);
	}

	equals(role) {
		return roleStringEquality(this._role, role._role) && this._scope.isSame(role._scope);
	}
